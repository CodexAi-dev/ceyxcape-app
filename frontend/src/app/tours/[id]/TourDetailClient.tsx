...
// Receives the tour from the SERVER component (page.tsx). All SEO content is
// already in the server-rendered HTML; this just hydrates the interactive UI.
export default function TourDetailClient({ tour }: { tour: Tour }) {
  ...
  const [mainImage, setMainImage] = useState(tour.image);

  // Count a view once on mount (fire-and-forget; never blocks render).
  useEffect(() => {
    tourService.incrementTourViews(tour.id).catch(() => {});
  }, [tour.id]);
